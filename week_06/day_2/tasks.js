...
let myArray = ['Lotr', 'Avengers', 'TbbT']
myArray[0] = 'Lotr: Two Towers'
myArray[1] = 'Avengers: Infinity War'
myArray.unshift("Matrix")
myArray.pop()
...
indexToRemove = programmingLanguages.indexOf("Visual Basic")
programmingLanguages.splice(indexToRemove, 1)
...
function addXtoTicTacToe(row,index,character='O'){
    ticTacToe[row][index] = character
}
addXtoTicTacToe(0,0,"X")
addXtoTicTacToe(1,1,"X")
addXtoTicTacToe(2,2,"X")
...
for (let i = 0; i <= 2; i++){
    ...
    intros.push(objectToPush)
}
for (let intro of intros){
    console.log(`Hi, my name is ${intro.firstName} ${intro.lastName} and I am in ${intro.band}`)
}
